	...
	{	for(n=lb1; n<ub; n++)
		{
			for(i=2; i<=n/2; i++)
				{if(n%i==0)
					{
					count++;
					break; /* vd (7,10) for 1: n=8, for 2 n%2=0 va i=2 gap break dung vong lap nen no thoat for 2 quay ve for 1 tang n=9 , 9%2!=0 tang i=3 chia het gap break, 
			...
			if (i> n/2)  //dieu kien nay de loc nhung n bi thoat ra khoi nhung vong lap phia tren (n gap break va n nguyen to, va dac biet la so 2)
				{		//i>n/2 thi k can kiem tra nua
				printf("So nguyen to dau tien trong khoang (%d, %d) la: %d", lb, ub, n);
				break; //de so nguyen to dau tien xong no break thoat khoi vong lap for thu 1. 
				}
		}if(count== dem)
		printf("Khong co so nguyen to nao trong khoang (%d, %d)", lb, ub);
